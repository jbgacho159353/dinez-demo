    ...
    <section className="py-20 px-4 sm:px-6 lg:px-8"><div className="max-w-7xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-20">
        <div>
          ...
          <h2 className="text-4xl font-playfair font-bold text-[#1A237E] mb-5">White-Glove Wedding Service</h2>
          <p className="text-gray-600 leading-relaxed mb-6">Your wedding day deserves nothing short of perfection. Our wedding chauffeur service is designed around your specific schedule, with dedicated drivers who understand the importance of every minute.</p>
          <p className="text-gray-600 leading-relaxed mb-8">From the bride's preparation address to the ceremony, reception and beyond — we coordinate seamlessly with your wedding planner and photographer to ensure flawless, beautiful arrivals.</p>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-7 shadow-sm border-t-4 border-t-gold">
          <h3 className="text-xl font-playfair font-bold text-[#1A237E] mb-5">Everything Included</h3>
          <div className="space-y-3">
            {includes.map(item=>(
              <div key={item} className="flex items-center gap-3"><svg className="w-5 h-5 text-gold shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg><span className="text-gray-700 text-sm">{item}</span></div>
            ))}
          </div>
        </div>
      </div>
      {/* Gallery */}
      <div className="text-center mb-10"><h2 className="text-4xl font-playfair font-bold text-[#1A237E]">Wedding Gallery</h2></div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {gallery.map((src,i)=>(
          <div key={i} className="relative h-52 rounded-xl overflow-hidden border border-gray-200 group shadow-sm">
