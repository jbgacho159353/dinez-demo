...
export default function AboutPage() {
  return (
    ...
    <section className="py-20 px-4 sm:px-6 lg:px-8"><div className="max-w-7xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-20">
        <div>
          ...
          <h2 className="text-4xl font-playfair font-bold text-white mb-5">Founded with a Vision, Built on Trust</h2>
          <p className="text-gray-400 leading-relaxed mb-4">In 2009, Dinez Carnay — a Filipino-British professional with a passion for hospitality and precision — founded Dinez Executive Taxis from Aldershot with a single Mercedes and an unwavering commitment to excellence.</p>
          ...
          <p className="text-gray-400 leading-relaxed mb-6">Sixteen years later, that founding philosophy remains unchanged. Every journey — whether a local ride or an international VIP transfer — receives the same personal attention to detail that Dinez himself set on day one.</p>
          <div className="flex items-center gap-6 p-5 bg-dark-card border border-gold/20 rounded-xl">
        ...
        <div className="bg-dark-card border border-gold/20 rounded-2xl p-8">
          ...
          <blockquote className="text-xl text-white font-playfair italic leading-relaxed mb-6">"Your Global Travel Certainty is not just a tagline — it is our daily commitment to every single passenger who trusts us with their journey."</blockquote>
          <div className="flex items-center gap-3"><div className="w-12 h-12 rounded-full bg-gold/20 border border-gold/30 flex items-center justify-center text-gold font-bold">DC</div><div><div className="text-white font-semibold">Dinez Carnay</div><div className="text-gray-500 text-sm">Founder, Dinez Executive Taxis</div></div></div>
        </div>
      </div>

      {/* Team */}
      <div className="mb-20">
        <div className="text-center mb-12"><h2 className="text-4xl font-playfair font-bold text-white mb-4">Our Team</h2><p className="text-gray-400">Every Dinez chauffeur is DBS-checked, professionally trained and committed to your comfort.</p></div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {team.map(m=>(
            <div key={m.name} className="bg-dark-card border border-dark-border rounded-2xl p-6 text-center hover:border-gold/30 transition-all">
              ...
              <div className="flex items-center justify-center gap-2 mb-1"><h3 className="text-white font-semibold">{m.name}</h3></div>
      ...
      {/* Timeline */}
      <div>
        <div className="text-center mb-12"><h2 className="text-4xl font-playfair font-bold text-white">Our Journey: 2009–2025</h2></div>
        <div className="relative">
          <div className="absolute left-1/2 -translate-x-px w-0.5 h-full bg-dark-border hidden sm:block" />
            ...
            {timeline.map((item,i)=>(
              <div key={item.year} className={`flex flex-col sm:flex-row gap-4 sm:gap-8 ${i%2===0?"sm:flex-row":"sm:flex-row-reverse"}`}>
                <div className="sm:flex-1 sm:text-right">
                  {i%2===0 ? <div className="bg-dark-card border border-dark-border rounded-xl p-5 hover:border-gold/30 transition-all"><div className="text-gold font-bold text-sm mb-1">{item.year}</div><div className="text-white font-semibold mb-1">{item.title}</div><div className="text-gray-400 text-sm">{item.desc}</div></div> : <div className="sm:hidden bg-dark-card border border-dark-border rounded-xl p-5"><div className="text-gold font-bold text-sm mb-1">{item.year}</div><div className="text-white font-semibold mb-1">{item.title}</div><div className="text-gray-400 text-sm">{item.desc}</div></div>}
                </div>
                ...
                <div className="sm:flex-1">
                  {i%2!==0 ? <div className="hidden sm:block bg-dark-card border border-dark-border rounded-xl p-5 hover:border-gold/30 transition-all"><div className="text-gold font-bold text-sm mb-1">{item.year}</div><div className="text-white font-semibold mb-1">{item.title}</div><div className="text-gray-400 text-sm">{item.desc}</div></div> : null}
                </div>
              </div>
            ))}
          </div>
    ...
    {/* Mission */}
    <section className="py-16 px-4 sm:px-6 lg:px-8 bg-dark-surface"><div className="max-w-3xl mx-auto text-center">
      <p className="text-gold text-xs uppercase tracking-[0.3em] font-semibold mb-3">Our Mission</p>
      <h2 className="text-4xl font-playfair font-bold text-white mb-6">Your Global Travel Certainty</h2>
