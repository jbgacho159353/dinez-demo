...
export default function ContactPage() {
  ...
  const input = "w-full bg-[#F8F9FA] border border-gray-300 rounded-xl px-4 py-3 text-[#333333] placeholder-gray-400 focus:outline-none focus:border-[#1A237E] focus:ring-1 focus:ring-[#1A237E] transition-colors";
  const label = "block text-sm text-gray-700 mb-1.5 font-medium";
  return (
    ...
    <section className="py-16 px-4 sm:px-6 lg:px-8"><div className="max-w-7xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
        ...
        <div className="lg:col-span-2 space-y-5">
          {[
            ...
            <div key={c.label} className="bg-white border border-gray-200 rounded-2xl p-5 flex items-start gap-4 hover:border-[#1A237E]/30 transition-all shadow-sm border-l-4 border-l-[#1A237E]">
              ...
              <div><div className="text-gray-500 text-xs uppercase tracking-wider mb-1">{c.label}</div>
                {c.href ? <a href={c.href} className="text-[#333333] hover:text-gold transition-colors text-sm font-medium">{c.value}</a> : <span className="text-[#333333] text-sm font-medium">{c.value}</span>}
              </div>
            </div>
          ))}
          <div className="bg-gold/5 border border-gold/20 rounded-2xl p-5">
            ...
            <div className="text-gray-700 text-sm">24 hours · 7 days a week · 365 days a year</div>
          </div>
        </div>
        {/* Form */}
        <div className="lg:col-span-3">
          {sent ? (
            <div className="bg-white border border-gray-200 rounded-2xl p-10 text-center shadow-sm">
              ...
              <h3 className="text-2xl font-playfair font-bold text-[#1A237E] mb-2">Message Sent!</h3>
              <p className="text-gray-600">Thank you {form.name}. We will reply to {form.email} within 2 hours.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-2xl p-8 space-y-5 shadow-sm">
              <h3 className="text-2xl font-playfair font-bold text-[#1A237E] mb-2">Send Us a Message</h3>
      ...
      {/* Map */}
      <div id="map" className="mt-10 rounded-2xl overflow-hidden border border-gray-200">
