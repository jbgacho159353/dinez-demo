...
export default function CruisePage() {
  return (
    ...
    <section className="py-20 px-4 sm:px-6 lg:px-8"><div className="max-w-7xl mx-auto">
      <div className="text-center mb-12"><h2 className="text-4xl font-playfair font-bold text-[#1A237E] mb-4">UK Cruise Ports We Serve</h2></div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {ports.map(p=>(
          <div key={p.name} className="bg-white border border-gray-200 rounded-2xl p-6 hover:border-gold/30 transition-all shadow-sm">
            <div className="text-3xl mb-3">🚢</div>
            <h3 className="text-[#1A237E] font-bold mb-1">{p.name}</h3>
    ...
    <section className="py-16 px-4 sm:px-6 lg:px-8 bg-[#F8F9FA]"><div className="max-w-5xl mx-auto">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 items-center">
        <div><h2 className="text-3xl font-playfair font-bold text-[#1A237E] mb-4">What's Included</h2>
          {["Door-to-terminal luggage assistance","Complimentary mineral water","DBS-checked, uniformed chauffeur","All port taxes and parking covered","Flight-delay style monitoring for cruise times","V-Class & Sprinter for larger groups"].map(i=>(
            ...
            <span className="text-gray-700 text-sm">{i}</span></div>
          ))}
        </div>
        <div className="bg-white border border-gray-200 rounded-2xl p-7 text-center shadow-sm">
          <div className="text-5xl mb-4">🚢</div>
          <h3 className="text-2xl font-playfair font-bold text-[#1A237E] mb-3">Group Travel?</h3>
          <p className="text-gray-600 text-sm mb-6">Our Mercedes V-Class seats 7 and our Sprinter handles up to 16. Perfect for cruise groups.</p>
