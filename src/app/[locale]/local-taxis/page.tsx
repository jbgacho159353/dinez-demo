    ...
    <main className="bg-white min-h-screen">
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-playfair font-bold text-[#1A237E] mb-4">Areas We Cover</h2>
            <p className="text-gray-600 max-w-2xl mx-auto">Local knowledge, luxury vehicles. We know every street, every shortcut, every station.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {areas.map(a => (
              <div key={a.town} className="bg-white border border-gray-200 rounded-2xl p-6 hover:border-gold/30 transition-all shadow-sm">
                ...
                <h3 className="text-[#1A237E] font-bold mb-2">{a.town}</h3>
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-[#F8F9FA]">
        <div className="max-w-5xl mx-auto text-center">
          <h2 className="text-4xl font-playfair font-bold text-[#1A237E] mb-4">Why We Are Different</h2>
          <p className="text-gray-600 mb-10">Every local journey receives the same five-star treatment as our airport and VIP transfers.</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[["🕐","Always Punctual"],["🌟","Five-Star Fleet"],["💷","Fixed Fares"],["📱","Easy Booking"]].map(([i,l]) => (
              <div key={l} className="bg-white border border-gray-200 rounded-2xl p-5 text-center hover:border-gold/30 transition-all shadow-sm">
                <div className="text-3xl mb-3">{i}</div>
                <div className="text-[#1A237E] text-sm font-medium">{l}</div>
              </div>
            ))}
          </div>
        </div>
